...
jsPsych.plugins['image-slider-response'] = (function() {
  ...
  jsPsych.pluginAPI.registerPreload('image-slider-response', 'stimulus_left', 'image');
  jsPsych.pluginAPI.registerPreload('image-slider-response', 'stimulus_centre', 'image');
  jsPsych.pluginAPI.registerPreload('image-slider-response', 'stimulus_right', 'image');
  ...
  plugin.trial = function(display_element, trial) {
    ...
    var html = '<center><div id="jspsych-image-slider-response-wrapper" style="margin: 100px 0px;">';
    ...
    // stimuli
    html += '<center>' +
    '<img id="jspsych-image-slider-response-multi2-stimulus_left" class="jspsych-image-slider-response-multi2-stimulus" src="'+trial.stimulus_left+'"></img>'+
    '<img id="jspsych-image-slider-response-multi2-stimulus_centre" class="jspsych-image-slider-response-multi2-stimulus" src="'+trial.stimulus_centre+'"></img>'+
    '<img id="jspsych-image-slider-response-multi2-stimulus_right" class="jspsych-cimage-slider-response-multi2-stimulus" src="'+trial.stimulus_right+'"></img>';
    ...
    html += '<input type="range" value="'+trial.start+'" min="'+trial.min+'" max="'+trial.max+'" step="'+trial.step+'" style="width: 50%;" id="jspsych-image-slider-response-response"></input>';
    html += '<div>'
    ...
    display_element.querySelector('#jspsych-image-slider-response-response').addEventListener('click', function() {
      ...
      // slider
      html += '<div class="jspsych-image-slider-response-container" style="position:relative;">';
      html += '<input type="range" value="'+curResponse+'" min="'+trial.min+'" max="'+trial.max+'" step="'+trial.step+'" style="width: 50%;" id="jspsych-image-slider-response-response"></input>';
      html += '<div>'
      for(var j=0; j < trial.labels.length; j++){
        var width = 100/(trial.labels.length-1);
        var left_offset = (j * (50 /(trial.labels.length - 1))) - (width/4);
